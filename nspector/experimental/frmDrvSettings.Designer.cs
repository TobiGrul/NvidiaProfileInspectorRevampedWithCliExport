...
namespace nspector
{
    ...
    internal partial class frmDrvSettings : Form
    {
        ...
        internal frmDrvSettings(bool showCsnOnly, bool skipScan)
        {
            ...
            // DarkMode: Apply dark mode right after all controls are initialized
            DarkTheme.Apply(this);

            // DarkMode: More control over colors
            ...
            // DarkMode: Set these ListView properties here where lvSettings is defined
            lvSettings.FullRowSelect = true;
            lvSettings.HideSelection = false;
        ...
        private void ListViewEx_DrawSubItem(object sender, DrawListViewSubItemEventArgs e)
        {
            ...
            // 1) Define background color based on selection
            Color bgColor = isSelected
                ? Color.FromArgb(0, 120, 215)  // selected → blue
                : Color.FromArgb(31, 31, 31);  // unselected → dark gray

            // 2) Define foreground color based on modified vs. stock
            Color fgColor = isSelected
                ? Color.White                  // selected → white
                : isChanged
                    ? Color.White              // changed → white
                    : Color.LightGray;         // unchanged → light gray
